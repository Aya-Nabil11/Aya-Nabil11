<div align="center">
  <img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&size=32&duration=2800&pause=2000&color=A855F7&center=true&vCenter=true&width=940&lines=Aya+Nabil+Alharazin;Teaching+Assistant+%7C+Data+Enthusiast;Building+Tech+for+Social+Impact+🇵🇸" alt="Typing SVG" />
</div>

<h3 align="center">Computer Science Graduate @ Islamic University of Gaza | GPA: 95.71/100 🎓</h3>

---

## 👩‍💻 Who is Aya?

I'm a **Computer Science graduate** from Gaza, Palestine, passionate about using technology and data to create meaningful change. Currently serving as a **Teaching Assistant** at the Faculty of Information Technology, I'm dedicated to empowering the next generation of developers while pursuing my own journey in **Data Science**.

### 🎯 My Focus Areas

- **Education**: Teaching Python, Java, Data Structures, and Operating Systems to university students
- **Data Science**: Building expertise in NumPy, Pandas, and data analysis for humanitarian applications
- **Social Impact**: Developing tech solutions that address real-world challenges in Gaza and beyond
- **Academic Growth**: Preparing for MSc in Data Science/Advanced Computer Science 

---

## 💼 What I Do

**Current Role: Teaching Assistant @ IUG**
- Guide students through Python, Java, Data Structures, and Operating Systems courses
- Create educational materials and practical coding exercises
- Mentor students in problem-solving and algorithmic thinking

**Technical Work:**
- Implementing data structures and algorithms
- Building data analysis projects with NumPy and Pandas
- Working with humanitarian datasets for evidence-based reporting
- Developing educational resources for IT students

---

## 🛠️ Technical Skills

**Languages:** Python • Java • C • SQL

**Data & Analysis:** NumPy • Pandas • Excel • Jupyter Notebook • Data Cleaning • CSV Processing

**Development Tools:** Git • GitHub • Linux (Ubuntu) • Docker • VirtualBox • VS Code

**Currently Learning:** APIs • Data Visualization • Humanitarian Data Analysis • Advanced Algorithms

---

## 📊 GitHub Stats

<div align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=Aya-Nabil11&show_icons=true&theme=radical&include_all_commits=true&count_private=true" alt="GitHub Stats" />
</div>

<div align="center">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=Aya-Nabil11&layout=compact&theme=radical&langs_count=6" alt="Top Languages" />
</div>

<div align="center">
  <img src="https://streak-stats.demolab.com?user=Aya-Nabil11&theme=radical" alt="GitHub Streak" />
</div>

---

## 🎯 My Mission

Using **data science** and **technology** to drive social and humanitarian impact. I believe in:

- **Education as empowerment** – Helping students unlock their potential through coding
- **Data-driven solutions** – Using analysis to inform humanitarian decisions
- **Tech for good** – Building applications that solve real problems in my community
- **Continuous growth** – Always learning, always improving

---

## 🚀 What You'll Find Here

- **📂 Data Structures & Algorithms**: Core CS implementations and teaching materials
- **📊 Data Science Projects**: Analysis work with NumPy, Pandas, and visualization
- **🧪 Educational Resources**: Labs, exercises, and materials for IT students  
- **🌍 Humanitarian Projects**: Gaza-focused and social impact initiatives

---

## 🌱 Current Journey

```
Computer Science Graduate (95.71 GPA)
           ↓
   Teaching Assistant @ IUG
           ↓
Advanced Data Science Learning
           ↓
   MSc Preparation 
           ↓
Data Scientist for Social Impact
```

**Goal:** Leverage data science to create evidence-based solutions for humanitarian challenges while continuing to educate and inspire the next generation of developers.

---

## 📫 Let's Connect

<div align="center">
  <a href="https://www.linkedin.com/in/aya-alharazin" target="_blank">
    <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" height="35" alt="linkedin"  />
  </a>
  <a href="mailto:alharazinaya@gmail.com" target="_blank">
    <img src="https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white" height="35" alt="gmail"  />
  </a>
  <a href="https://www.youtube.com/@AyaAlharazin" target="_blank">
    <img src="https://img.shields.io/badge/YouTube-FF0000?style=for-the-badge&logo=youtube&logoColor=white" height="35" alt="youtube"  />
  </a>
  <a href="https://www.instagram.com/alharazinaya/" target="_blank">
    <img src="https://img.shields.io/badge/Instagram-E4405F?style=for-the-badge&logo=instagram&logoColor=white" height="35" alt="instagram"  />
  </a>
</div>

---

<div align="center">
  
**"Code + coffee = life"**

![Profile Views](https://komarev.com/ghpvc/?username=Aya-Nabil11&style=flat-square&color=blueviolet)

</div>
